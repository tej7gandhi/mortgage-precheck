...
try:
    ...
    _startup_registry = load_kb_registry()
...
# ============================================================================
def rebuild_kb_registry_from_qdrant() -> dict:
    ...
    try:
        ...
        while True:
            ...
            for p in points:
                ...
                # support both nested metadata and flat
        ...
        # If collection empty or not ready, return empty
    ...
    if KB_REGISTRY.exists():
        try:
            with open(KB_REGISTRY) as f:
                data = json.load(f)
                if data:
                    # sanity: if file exists but Qdrant has more docs, sync
                    # quick count check
                    return data
        except Exception:
            pass
    # fallback rebuild
    return rebuild_kb_registry_from_qdrant()
...
with tab_kb:
    ...
    with col_head2:
        if st.button("🔄 Sync from Qdrant", help="Rebuild local registry from Qdrant Cloud if it was wiped"):
            with st.spinner("Syncing from Qdrant..."):
                registry_synced = rebuild_kb_registry_from_qdrant()
                st.success(f"Synced {len(registry_synced)} docs, {sum(v['chunk_count'] for v in registry_synced.values())} chunks from Qdrant")
                st.rerun()
    registry = load_kb_registry()
