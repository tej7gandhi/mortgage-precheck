"""
MORTGAGE UNDERWRITING APP - V9 FINAL - NO REFRESH NEEDED
Fixes: Blink loop + refresh required workaround
- Upload no longer triggers st.rerun() - stays on same page
- Uploader clears via key increment without rerun loop
- Gap Analysis button works immediately after upload
- All previous fixes: Qdrant persistence, re-upload replace, 401, 413
"""
...
with tab2:
    ...
    # V9 FIX: No auto-rerun loop - process uploads immediately but keep button active
    # Use form to prevent immediate rerun on file select
    with st.form(key=f"upload_form_{app['app_id']}_{st.session_state.uploader_nonce}", clear_on_submit=True):
        uploaded_files = st.file_uploader("Select PDFs to add/replace (same name = replace, new name = add)", type=["pdf"], accept_multiple_files=True)
        ...
        if submit_upload and uploaded_files:
            ...
            if files_processed:
                ...
                st.session_state.current_app = app  # Update session state immediately
                st.success(f"✅ Saved {len(files_processed)} file(s) to Qdrant: {', '.join(files_processed)}. Total chars: {len(app['evidence_text'])}. You can now click Run Gap Analysis below - no refresh needed!")
                st.session_state.uploader_nonce += 1
                # NO st.rerun() here - let user see success and click gap analysis
    
    if app["evidence_files"]:
        with st.expander(f"📂 Evidence Files ({len(app['evidence_files'])}) - Current in Qdrant", expanded=True):
            for i, fname in enumerate(app["evidence_files"], 1):
                count = app["evidence_text"].count(f"--- {fname} (Page")
                st.write(f"{i}. 📄 {fname} ({count} pages)")
            st.caption(f"Total evidence: {len(app['evidence_text'])} chars")
    ...
    if not kb_registry:
        ...
        st.caption(f"Will analyze {len(app['evidence_files'])} files, {len(app['evidence_text'])} chars. This will include any newly uploaded gap-cover docs.")
        # Button outside form so it works independently
        if st.button("🚀 Run Gap Analysis (uses latest uploaded data - no refresh needed)", use_container_width=True, type="primary", key=f"gap_btn_{app['app_id']}_{len(app['evidence_files'])}_{len(app['analysis_history'])}"):
            with st.spinner("Analysing latest evidence (including re-uploaded gap cover docs)... 20-30 sec..."):
                try:
                    ...
                    st.success("✅ Analysis complete - gaps re-evaluated with new docs!")
                    st.rerun()
                except Exception as e:
    ...
    if app["analysis_history"]:
        ...
        st.caption(f"{latest['timestamp'][:19].replace('T', ' at ')} • {latest['evidence_count']} files • {latest.get('evidence_text_length', 0)} chars")
        ...
        if len(app["analysis_history"]) > 1:
            with st.expander(f"📜 Previous Runs"):
