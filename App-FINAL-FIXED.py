...
def retrieve_rules(state: UnderwritingState) -> dict:
    ...
    if vs_curated:
        try:
            curated_docs = vs_curated.similarity_search(query, k=6)
            curated_text = "\n\n".join(d.page_content for d in curated_docs)
        except:
            pass
...
List what is present, what is missing, and specific gaps that prevent approval. Be specific about document types."""

    user = f"""
BORROWER EVIDENCE FILES: {', '.join(state['evidence_files'])}
EVIDENCE TEXT (truncated):
{state['evidence_text'][:12000]}

FANNIE MAE RULES:
{state['fnma_rules'][:8000]}

FHA RULES:
{state['fha_rules'][:8000]}
...
def recommend_guideline(state: UnderwritingState) -> dict:
    system = """You are a senior mortgage underwriter making final recommendation.
Based on gap analysis, recommend Fannie Mae vs FHA and explain why. Provide clear next steps."""

    user = f"""
EVIDENCE: {state['evidence_files']}
GAP ANALYSIS:
{state['gap_analysis']}

FANNIE MAE RULES: {state['fnma_rules'][:4000]}
FHA RULES: {state['fha_rules'][:4000]}

Return in markdown:
### 🏛️ Recommendation: [Fannie Mae / FHA / Neither - Need More Docs]
